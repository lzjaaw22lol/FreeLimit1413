...
@interface FreeViewController ()<UITableViewDataSource,UITableViewDelegate,MJRefreshBaseViewDelegate,UISearchBarDelegate,ASIHTTPRequestDelegate>
{
    UITableView *tbv;
...
-(void)getData:(int)n{
    ...
    if(n == 1){
        pagecount = 1;
        NSString *urlstr = [NSString stringWithFormat:FREE_URL,1,@"6014"];
        ...
        pagecount++;
        NSLog(@"%d",pagecount);
        NSString *urlstr = [NSString stringWithFormat:FREE_URL,pagecount,@"6014"];
        NSURL *url = [NSURL URLWithString:urlstr];
        ASIHTTPRequest *request = [[ASIHTTPRequest alloc] initWithURL:url];
        request.delegate = self;
        request.tag = 2;
