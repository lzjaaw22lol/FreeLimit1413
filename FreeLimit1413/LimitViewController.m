...
@interface LimitViewController ()<UITableViewDataSource,UITableViewDelegate,MJRefreshBaseViewDelegate,UISearchBarDelegate,ASIHTTPRequestDelegate>
{
    UITableView *tbv;
...
-(void)getData:(int)n{
    ...
    if(n == 1){
        pagecount = 1;
        NSString *urlstr = [NSString stringWithFormat:LIMIT_URL,1,@"6014"];
        ...
        pagecount++;
        NSLog(@"%d",pagecount);
        NSString *urlstr = [NSString stringWithFormat:LIMIT_URL,pagecount,@"6014"];
        NSURL *url = [NSURL URLWithString:urlstr];
        ASIHTTPRequest *request = [[ASIHTTPRequest alloc] initWithURL:url];
        request.delegate = self;
        request.tag = 2;
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if(request.tag == 1){
        ...
        NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:request.responseData options:0 error:nil];
        if([dict objectForKey:@"applications"]){
            NSLog(@"dd:%lu",(unsigned long)[[dict objectForKey:@"applications"] count]);
            [muArr addObjectsFromArray:[dict objectForKey:@"applications"]];
            fullMsg = [NSArray arrayWithArray:muArr];
...
-(void)getStar:(NSString *)st and:(UIView *)vi{
    ...
    for(int j=0;j<5-nostar;j++){
        UIImageView *p = [[UIImageView alloc] initWithFrame:CGRectMake(5+15*i+15*j, 3, 15, 15)];
        p.image = [UIImage imageNamed:@"1.主页_16.png"];
        [vi addSubview:p];
    }
